...
public class RMIServerImplWrapper extends ClassAnnotationGlobalHandler {
	com.docker.rpc.impl.RMIServerHandler rmiServerHandler;
	//Server
	Map<String, GroovyObjectEx<RPCServerAdapter>> serverAdapterMap;

	Integer port;

	com.docker.rpc.impl.RMIServer server;

	RPCServerMethodInvocation serverMethodInvocation;

	public com.docker.rpc.impl.RMIServer initServer(boolean enableSsl) {
		if(server == null) {
			try {
				if (enableSsl) {
					server = new com.docker.rpc.impl.RMIServerImpl(port + 1, this, enableSsl);
				} else {
	...
	public void initClient(com.docker.rpc.impl.RMIServer server) {
		this.server = server;
	}
...
//		super(port + 1);
	...
	public com.docker.rpc.impl.RMIServerHandler getRmiServerHandler() {
