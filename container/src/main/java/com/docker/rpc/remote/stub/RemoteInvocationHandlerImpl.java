...
public class RemoteInvocationHandlerImpl implements com.docker.rpc.remote.stub.RemoteInvocationHandler {
    private final String TAG = RemoteInvocationHandlerImpl.class.getSimpleName();
    private com.docker.rpc.remote.stub.RemoteServerHandler remoteServerHandler;
    ...
    private void handleAsyncWithHandler(MethodMapping methodMapping, MethodRequest methodRequest) {
        if (methodMapping.getAsync()) {
            ...
            com.docker.rpc.remote.stub.RpcCacheManager.getInstance().pushToAsyncRpcMap(asyncRpcFuture.getCallbackFutureId(), asyncRpcFuture);
