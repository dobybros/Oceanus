...
public class RPCClientAdapterMap {
    ...
    private ConcurrentHashMap<String, com.docker.rpc.RPCClientAdapter> clientAdapterMap = new ConcurrentHashMap<String, com.docker.rpc.RPCClientAdapter>();
    ...
    public com.docker.rpc.RPCClientAdapter registerServer(String ip, int rmiPort, final String serverName) {
        return registerServer(ip, rmiPort, serverName, null);
    }

    public com.docker.rpc.RPCClientAdapter getClientAdapter(String serverName) {
        return clientAdapterMap.get(serverName);
    }

    public com.docker.rpc.RPCClientAdapter registerServer(String ip, int rmiPort, final String serverName, com.docker.rpc.RPCClientAdapter.ClientAdapterStatusListener statusListener) {
        if (serverName == null)
            return null;
        com.docker.rpc.RPCClientAdapter clientAdapter = clientAdapterMap.get(serverName);
...
//			rmiClient.setServerAdapterMap(serverAdapterMap);
            rmiClient.setExpireTime(expireTime, new ExpireListener<com.docker.rpc.RPCClientAdapter>() {
                @Override
                public boolean expired(com.docker.rpc.RPCClientAdapter handler, long touch, long expireTime) {
                    com.docker.rpc.RPCClientAdapter removedHandler = RPCClientAdapterMap.this.unregisterServer(serverName);
            ...
            com.docker.rpc.RPCClientAdapter existingClientAdapter = clientAdapterMap.putIfAbsent(serverName, clientAdapter);
    ...
    public com.docker.rpc.RPCClientAdapter unregisterServer(String serverName) {
        com.docker.rpc.RPCClientAdapter handler = clientAdapterMap.remove(serverName);
        if (handler != null) {
            handler.clientDestroy();
        }
        return handler;
    ...
    public Map<String, com.docker.rpc.RPCClientAdapter> getClientAdapterMap() {
