...
public class DockerStatus extends DataObject {
    ...
    public static final String FIELD_DEPLOYID = "deployId";
    public static final String FIELD_FAILEDREASON = "failedReason";
    /**
    ...
    private List<com.docker.data.Service> services;
    ...
    public static final int STATUS_OK = 1; //start success
    public static final int STATUS_STARTING = 10; //in start
    public static final int STATUS_WAITSTART = 30; //wait start
    public static final int STATUS_PAUSE = 40;//GATEWAY AND PROXY
    ...
    public void fromDocument(Document dbObj) {
        ...
        deployId = dbObj.getString(FIELD_DEPLOYID);
        failedReason = dbObj.getString(FIELD_FAILEDREASON);
        ...
        if (servicesList != null) {
            ...
            for (Document serviceDoc : servicesList) {
                com.docker.data.Service service = new com.docker.data.Service();
    ...
    public Document toDocument() {
        ...
        if (deployId != null) {
            dbObj.put(FIELD_DEPLOYID, deployId);
        }
        if(failedReason != null){
            dbObj.put(FIELD_FAILEDREASON, failedReason);
        }
        if (services != null) {
            List<Document> serviceList = new ArrayList<>();
            for (com.docker.data.Service service : services) {
    ...
    public List<com.docker.data.Service> getServices() {
        return services;
    }
