...
public class GetTheAppender {
    public static ConsoleAppender getConsoleAppender() {
        ...
        patternLayoutEncoder.setPattern("%date{yyyy-MM-dd HH:mm:ss.SSS} %-5level[%thread](%file:%line\\): %msg%n");
...
//        ConsoleAppender consoleAppender = new ConsoleAppender();
        ...
        path = Paths.get(path, "../../../logs_starfish/").toString();
        String dateStr = format.format(new Date());
        //设置文件名
        appender.setFile(OptionHelper.substVars(path + "/" + name + "/" + dateStr + "/" + level.levelStr + ".log", context));
        ...
        //文件名格式
        String fp = OptionHelper.substVars(path + "/" + name + "/" + dateStr + "/" + level.levelStr + "/.%d{yyyy-MM-dd}.%i.log", context);
        //最大日志文件大小
