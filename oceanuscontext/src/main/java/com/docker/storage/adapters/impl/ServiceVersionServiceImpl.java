...
//                .append("serverType." + serverType, new Document().append("$exists", true));
    ...
    public void addServiceVersion(ServiceVersion serviceVersion) throws CoreException {
        if (serviceVersion.getId() == null) {
            serviceVersion.setId(ObjectId.get().toString());
        }
        try {
            serviceVersionDAO.updateOne(new Document().append("_id", serviceVersion.getId()), new Document().append("$set", serviceVersion.toDocument()), true);
        } catch (DBException e) {
            e.printStackTrace();
