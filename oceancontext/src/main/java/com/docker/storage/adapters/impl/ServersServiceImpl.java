...
public class ServersServiceImpl implements ServersService {
    ...
    public Document getServerConfig(String service) throws CoreException {
        try {
            FindIterable<Document> iterable = serversDAO.query(new Document().append("_id", service));
            MongoCursor<Document> cursor = iterable.iterator();
            if(cursor.hasNext())
                return cursor.next();
        } catch (DBException e) {
            e.printStackTrace();
            throw new CoreException(ChatErrorCodes.ERROR_ONLINESERVER_QUERY_FAILED, "Query server config for " + service + " failed, " + e.getMessage());
        }
        return null;
    }
