package com.proxy.runtime.executor.prepare.source;
...
import com.proxy.runtime.DefaultRuntimeContext;
 ...
 * Description：
 */
public class GridFSServiceDownloadHandler implements ServiceDownloadHandler {
    private final String TAG = GridFSServiceDownloadHandler.class.getName();
    @Override
    public Boolean prepare(Configuration configuration) throws CoreException, IOException {
        ...
        FileAdapter.FileEntity fileEntity
                = fileAdapter.getFileEntity(new FileAdapter.PathEx(configuration.getBaseConfiguration().getRemotePath() + configuration.getServiceVersion() + File.separator + configuration.getFileName()));
        if(fileEntity == null){
